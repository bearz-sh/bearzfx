...
public class EvaluateCommand : Command
{
    ...
    {
        this.AddArgument(new Argument<string>("template", "The template directory"));
        this.AddOption(new Option<string?>(new[] { "--env", "-e" }, "The environment to evaluate"));
...
public class EvaluateCommandHandler : ICommandHandler
{
    ...
    public async Task<int> InvokeAsync(InvocationContext context)
    {
        ...
        {
            Environment = env,
            TemplateDirectory = this.Template,
            Import = this.Import,
