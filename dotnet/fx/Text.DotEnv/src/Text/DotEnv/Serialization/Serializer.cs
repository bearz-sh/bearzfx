...
public static class Serializer
{
    ...
    public static void Serialize<T>(Stream stream, T obj, DotEnvSerializerOptions? options = null)
    {
        using var sw = new StreamWriter(stream, Encoding.UTF8, -1, true);
        Serialize(stream, obj, typeof(T), options);
    }

    public static void Serialize<T>(TextWriter writer, T obj, DotEnvSerializerOptions? options = null)
    {
        Serialize(writer, obj, typeof(T), options);
    }

    public static string Serialize(object? obj, Type type, DotEnvSerializerOptions? options = null)
    {
        using var sw = new StringWriter();
    ...
    {
        var dict = (IDictionary<string, string>)Activator.CreateInstance<T>();
        foreach (var (name, value) in document.AsNameValuePairEnumerator())
            dict.Add(name, value);
