...
public class ZeVariables
{
    ...
    public ZeVariables Add(IConfiguration config)
    {
        ProcessSection(config.GetChildren(), this.variables);
        return this;
    }

    public ZeVariables Add(IPathSpec paths)
    {
        this.variables["paths"] = new Dictionary<string, object?>()
        {
            ["plank"] = paths.ZeRootDir,
            ["etc"] = paths.EtcDir,
            ["data"] = paths.DataDir,
            ["log"] = paths.LogDir,
            ["run"] = paths.RunDir,
    ...
    public ZeVariables Add(ZeSpec spec)
    {
    ...
    public ZeVariables AddVarFiles(IReadOnlyCollection<string> varFiles)
    {
        if (varFiles.Count == 0)
        {
            return this;
    ...
    public ZeVariables LoadAsEnvironmentVariables()
    {
